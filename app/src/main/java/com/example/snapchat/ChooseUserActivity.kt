    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        FirebaseDatabase.getInstance().getReference().child("users").addChildEventListener(object : ChildEventListener{
            override fun onChildAdded(p0: DataSnapshot, p1: String?) {
                val email:String = p0.child("email").value as String
                emails.add(email)
                keys.add(p0.key!!)
        ...
        chooseUserListView?.onItemClickListener = AdapterView.OnItemClickListener{parent: AdapterView<*>?, view: View?, position: Int, id: Long ->
            val snapMap:Map<String,String> = mapOf("from" to FirebaseAuth.getInstance().currentUser!!.email!!,"imageName" to intent.getStringExtra("imageName"),"caption" to intent.getStringExtra("caption"))
